...
class fv_tc extends fv_tc_Plugin {
    ...
    function cache_start( $args ) {
      ...
      $iComments = get_comments_number();
      $bCommentAuthor = false;
      $sCommentAuthor = false;
      ...
      if( $wp_query->comments ) {
        foreach( $wp_query->comments as $objComment ) {
          if( $objComment->comment_author_email == $sCommentAuthor && $objComment->comment_approved == 0 ) {
      ...
      if( !$bCommenterUnapproved ) {
        $bCommentAuthor = false;
        echo "<!--fv comments cache - no unapproved comments for $sCommentAuthor - serving cached data -->\n";
      } else {
        echo "<!--fv comments cache - unapproved comments for $sCommentAuthor - not serving cached data -->\n";
      }
      
      $cache = false;
      $filename = $post->ID.'-'.$post->post_name.'.tmp';
      $file = WP_CONTENT_DIR . '/fv-tc-cache-'.$blog_id.'/'.$filename;  //  check if exists!
      ...
      if( !is_user_logged_in() && !$bCommentAuthor && isset($cache['html']) && ($cache['date'] + 7200) > date( 'U' ) && isset($cache['comments']) && $cache['comments'] == $iComments && !isset( $_COOKIE['fv-debug'] ) ) {
        echo $cache['html'];
        echo "<!--fv comments cache from $filename @ ".$cache['date']."-->\n";
        
        //  skip the rest of the output!
        $args['walker'] = new FV_TC_Walker_Comment_blank;
      } else {echo "<!--fvtc FV_TC_Walker_Comment_capture -->\n";
...
//  todo: no better way?
/*function fv_tc_comment_callback_blank( $comment, $args, $depth ) {
...
class FV_TC_Walker_Comment_capture extends Walker_Comment {
    ...
    echo "<!--fvtc end 1-->\n";
    if( $depth == 0 ) {echo "<!--fvtc end 2-->\n";
      $cache['html'] = ob_get_clean();
      $cache['date'] = date( 'U' );
      $cache['comments'] = $iComments;
      
      echo $cache['html'];
  
      if( !is_user_logged_in() && !$bCommentAuthor && !isset( $_COOKIE['fv-debug'] ) ) {
        echo "<!--fv comments cache into $filename @ ".$cache['date']."-->\n";
        file_put_contents( $file, serialize( $cache ) );
      } else {
        echo "<!--fv comments do not cache into $filename @ ".$cache['date']." - user logged in or comment author-->\n";
      }   
    }    
	}
