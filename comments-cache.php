 ...
 * @see Walker
 */
class Walker_Comment_with_Cache extends Walker {
    ...
    if( $_GET['comment_cache'] == 'db' ) {
      ...
      if( !isset($this->aComments) ) {
        ...
        $this->aComments = $wpdb->get_results( "SELECT comment_ID, comment_content FROM {$wpdb->prefix}comments_cache WHERE comment_post_ID = $comment->comment_post_ID ", OBJECT_K );
      }
      
      global $wpdb;
      if( isset($this->aComments[$comment->comment_ID]) ) {
        $output .= $this->aComments[$comment->comment_ID]->comment_content;
        return;
      }
    }
